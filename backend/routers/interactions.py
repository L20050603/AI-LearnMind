...
def create_event(payload: InteractionEventCreate, db: Session = Depends(get_db)):
    event = log_event(
        db,
        payload.type,
        name=payload.name,
...
def list_events(db: Session = Depends(get_db)):
    return db.query(InteractionEvent).order_by(InteractionEvent.id.desc()).limit(100).all()
