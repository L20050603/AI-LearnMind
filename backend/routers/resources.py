...
from models import LearningResource, LearningTask, ResourceCrawlLog, ResourceRecommendation
...
@router.post("/search")
def search_resources(payload: ResourceSearchPayload, db: Session = Depends(get_db)):
    ...
    for index, resource in enumerate(saved[:5]):
        db.add(ResourceRecommendation(user_id=1, resource_id=resource.id, knowledge_point_id=resource.related_knowledge_point_id, reason="资源质量较高，适合当前复习目标", priority=max(10, resource.quality_score - index)))
    log_event(db, "resource", name="search_resource", action="search", page="ResourceHunter", target_id=payload.knowledgePointId, metadata={"queries": queries, "count": len(saved)})
    db.commit()
...
@router.post("/crawl")
def crawl_resource(payload: ResourceCrawlPayload, db: Session = Depends(get_db)):
    ...
    log_event(db, "resource", name="crawl_resource", action="crawl", page="ResourceHunter", target_id=payload.knowledgePointId, metadata={"url": payload.url})
    db.commit()
...
def recommendations_today(db: Session = Depends(get_db)):
    return [serialize_resource(item) for item in today_recommendations(db)]
...
def toggle_favorite(resource_id: int, db: Session = Depends(get_db)):
    ...
    log_event(db, "resource", name="favorite_resource", action="favorite" if resource.is_favorite else "unfavorite", page="ResourceHunter", target_id=resource_id)
    db.commit()
    return serialize_resource(resource)


@router.post("/{resource_id}/add-to-plan")
def add_resource_to_plan(resource_id: int, db: Session = Depends(get_db)):
    resource = db.query(LearningResource).filter(LearningResource.id == resource_id).first()
    if not resource:
        raise HTTPException(status_code=404, detail="Resource not found")
    resource.added_to_plan = True
    task = LearningTask(user_id=1, title=f"学习资源：{resource.title}", knowledge_point_id=resource.related_knowledge_point_id, difficulty="hard" if resource.difficulty_level in {"hard", "boss"} else "normal", estimated_minutes=resource.estimated_minutes, due_date="")
    db.add(task)
    log_event(db, "resource", name="add_resource_to_plan", action="add_to_plan", page="ResourceHunter", target_id=resource_id, metadata={"task_title": task.title})
    db.commit()
    db.refresh(task)
...
def generate_card(payload: ResourceCardPayload, db: Session = Depends(get_db)):
    resource = db.query(LearningResource).filter(LearningResource.id == payload.resourceId).first()
    if not resource:
        raise HTTPException(status_code=404, detail="Resource not found")
    card = build_learning_card(resource)
    log_event(db, "resource", name="generate_resource_card", action="generate_card", page="ResourceHunter", target_id=resource.id)
    db.commit()
    return card


@router.post("/{resource_id}/generate-quiz")
def generate_resource_quiz(resource_id: int, db: Session = Depends(get_db)):
    resource = db.query(LearningResource).filter(LearningResource.id == resource_id).first()
    if not resource:
        raise HTTPException(status_code=404, detail="Resource not found")
    quiz, mode = create_quiz(db, resource.related_knowledge_point_id, source_type="resource", source_id=resource.id, count=5)
    log_event(db, "quiz", name="generate_resource_quiz", action="generate", page="ResourceHunter", target_id=resource.id, metadata={"quiz_id": quiz.id, "mode": mode})
    db.commit()
