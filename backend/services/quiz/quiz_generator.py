...
def _context(db, knowledge_point_id, user_id: int | None = None):
    level = next((node for node in get_knowledge_nodes(db, user_id) if node["id"] == knowledge_point_id), None)
    return {"selected_level": level, "risk": evaluate_risk(db, persist=False, user_id=user_id), "sources": []}
...
def generate_questions(db, knowledge_point_id: int, count=5, user_id: int | None = None):
    ...
    try:
        questions, mode = call_provider("generate_quiz", topic, _context(db, knowledge_point_id, user_id), count)
        if not questions:
