...
export default function FocusControlPanel({
  ...
  return (
    <div className="glass-panel p-5">
      <KnowledgePointSelect value={selectedLevel?.id} onChange={(id) => setSelectedLevel(learningMap.find((node) => node.id === id))} label="专注知识点" />
      ...
      <div className="mt-5">
        ...
        <div className="mt-3 flex items-center gap-2">
          <input
            ...
            placeholder="自定义分钟"
            className="w-full rounded-2xl border border-white/10 bg-slate-950/80 px-3 py-2 text-sm text-white outline-none focus:border-cyan-300/70"
          />
        </div>
      </div>
      ...
      <div className="mt-5 rounded-3xl border border-white/10 bg-white/[0.045] p-4">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-300">{selectedLevel?.strategy || "专注完成后会写入学习记录，并触发掌握度与风险重新计算。"}</p>
      </div>

      <div className="mt-5 grid gap-2">
        {!isActive && (
          <button type="button" onClick={onStart} disabled={busy || !selectedLevel} className="primary-submit">
            {busy ? "启动中..." : "开始专注"}
          </button>
        )}
        ...
        {isActive && (
          <div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={onFinish} disabled={busy} className="primary-submit">
              完成
            </button>
