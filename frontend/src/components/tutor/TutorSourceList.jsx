export default function TutorSourceList({ sources = [] }) {
  return (
    <div className="space-y-3">
      {!sources.length && <p className="text-sm text-slate-400">No source snippets yet.</p>}
      {sources.map((source) => (
        <div key={source.id || source.title} className="rounded-2xl border border-white/10 bg-white/[0.045] p-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="rounded-full bg-cyan-400/10 px-2 py-0.5 text-[11px] text-cyan-100">score {source.score ?? 0}</span>
          </div>
