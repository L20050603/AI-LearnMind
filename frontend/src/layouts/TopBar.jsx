    ...
    <header className="sticky top-0 z-30 border-b border-white/10 bg-slate-950/60 px-4 py-3 backdrop-blur-2xl md:px-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm">
          <span className="nav-chip">{student?.name || "李同学"}</span>
          <span className="nav-chip">Lv.{student?.level ?? 7}</span>
          <span className="nav-chip">{student?.xp ?? 2680} XP</span>
          <span className="nav-chip hidden sm:flex">
            <Sparkles size={14} />
